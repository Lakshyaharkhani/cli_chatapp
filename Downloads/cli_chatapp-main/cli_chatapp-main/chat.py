                    ...
                    if current_count > last_count:
                        ...
                        if messages:
                            ...
                            if PLYER_AVAILABLE and not self.is_window_focused():
                                senders = [m.get("username", "Unknown") for m in messages if m.get("username") != self.username]
                                if senders:
                                    try:
                                        notification.notify(
                                            title="Redis Chat",
                                            message=f"New message from {senders[0]}" + (f" (+{len(senders)-1} others)" if len(senders) > 1 else ""),
